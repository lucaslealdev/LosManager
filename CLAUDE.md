...
**Update checking (`utils/atualizacao.py`)**: the GitHub Actions workflow (`.github/workflows/build-release.yml`) writes the run number to `assets/versao.txt` (gitignored — only exists inside a CI-built `.exe`, never in source or hand-built copies) right before the PyInstaller step, so it gets bundled and is readable at runtime via `config.caminho_asset("versao.txt")`. On startup, `main.py` calls `atualizacao.verificar_silenciosamente()` (no-op unless `sys.frozen`, i.e. running from source never checks) which hits `https://api.github.com/repos/ramonxxl/LosManager/releases/latest` on a background thread and only interrupts the user (`messagebox.askyesno` → `webbrowser.open`) if a newer build actually exists; any network/parse failure is swallowed on purpose so a launch never breaks over connectivity. `screens/configuracoes.py` also exposes `atualizacao.verificar_manualmente()` behind a button, which — unlike the silent variant — always calls back with either a result or an error, since the user asked for it explicitly. **This requires the GitHub repo to stay public**: the release API 404s for unauthenticated requests against a private repo, which would make every check silently fail forever with no way to tell from inside the running app.
